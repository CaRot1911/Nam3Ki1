...
namespace QuanLySinhVien
{
    public partial class ThongTinSinhVien : Form
    {
        ...
        private void grCon1_Click(object sender, EventArgs e)
        {
            tBox(true);
            Reset();
            bttSuaTT.Enabled = false;
        ...
        private void bttThemSV_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                data.DataSource = connectDataBase.DataReader("SELECT * FROM Student WHERE id = " + tbMSV.Text);
        ...
        private void bttXuat_Click(object sender, EventArgs e)
        {
            if (data.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < data.Rows.Count; i++)
                {
                    ...
                    exSheet.get_Range("A" + (i + 4).ToString()).Value = data.Rows[i].Cells["ID"].Value;
                    exSheet.get_Range("B" + (i + 4).ToString()).Value = data.Rows[i].Cells["fullName"].Value;
                    exSheet.get_Range("C" + (i + 4).ToString()).Value = data.Rows[i].Cells["address"].Value;
                    exSheet.get_Range("D" + (i + 4).ToString()).Value = data.Rows[i].Cells["gender"].Value;
                    exSheet.get_Range("E" + (i + 4).ToString()).Value = data.Rows[i].Cells["numberPhone"].Value;
                    exSheet.get_Range("F" + (i + 4).ToString()).Value = data.Rows[i].Cells["classID"].Value;
                    exSheet.get_Range("G" + (i + 4).ToString()).Value = data.Rows[i].Cells["dateOfBirth"].Value;
                    exSheet.get_Range("H" + (i + 4).ToString()).Value = data.Rows[i].Cells["specialized"].Value;
